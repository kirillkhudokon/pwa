...
/* ------------------------------------- */
const postsBox = document.querySelector('.blog-box');

if(postsBox){
  fetch('http://localhost:3001/posts', { headers: {
    'X-SWR-ID': 'app.posts.all'
  } })
    .then(r => r.json())
    .then(updatePosts);
}
...
function updatePosts(posts){
  if(postsBox){
    postsBox.innerHTML = `<pre>${JSON.stringify(posts.pagination, null, 2)}</pre>`
  }
}
